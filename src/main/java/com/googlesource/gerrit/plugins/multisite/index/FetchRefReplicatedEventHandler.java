...
public class FetchRefReplicatedEventHandler implements EventListener {
        ...
        try (Repository repo = gitRepositoryManager.openRepository(projectNameKey)) {
          if (changeId != null) {
            ...
            if ((RefUpdate.Result.FORCED.equals(fetchRefReplicatedEventResult)
              ...
              if(isMetaRef) {
                reindexMetaRef(projectNameKey, changeId, () -> changeIndexer.delete(changeId));
              } else {
              ...
              logger.atInfo().log(
                  ...
                  changeId, refName, projectNameKey.get());
              if(isMetaRef) {
                reindexMetaRef(projectNameKey, changeId, () -> changeIndexer.index(projectNameKey, changeId));
              } else {
  ...
  private void reindexMetaRef(Project.NameKey projectNameKey, Change.Id changeId, Runnable lambda) {
