...
public class ForwardedIndexChangeHandler
  ...
  protected void doIndex(String id, Optional<ChangeIndexEvent> indexEvent) {
    indexEvent.ifPresent(
        indexChangeEvent -> {
          if (ChangeIndexEvent.isAllChangesDeletedForProject(indexChangeEvent)) {
            indexer.deleteAllForProject(Project.nameKey(indexChangeEvent.projectName));
          } else {
