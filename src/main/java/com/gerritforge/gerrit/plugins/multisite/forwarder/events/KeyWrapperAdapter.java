...
public class KeyWrapperAdapter implements JsonSerializer<Object>, JsonDeserializer<Object> {
  ...
  public Object deserialize(JsonElement json, Type type, JsonDeserializationContext context)
    ...
    Map<String, Class<?>> cacheDefMap = getCacheKeyTypeByClass();
    if (json.isJsonObject()) {
      ...
      if (cacheDefMap == null || !cacheDefMap.containsKey(typeName)) {
  ...
  private static Optional<String> getCacheKeyTypeByClass(Class<?> clazz) {
    return getCacheKeyTypeByClass().entrySet().stream()
        .filter(entry -> entry.getValue().isAssignableFrom(clazz))
        .map(Map.Entry::getKey)
        .findFirst();
  }

  private static Map<String, Class<?>> getCacheKeyTypeByClass() {
