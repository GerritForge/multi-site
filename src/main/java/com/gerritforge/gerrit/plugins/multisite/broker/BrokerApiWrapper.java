...
public class BrokerApiWrapper implements BrokerApi {
  ...
  public ListenableFuture<Boolean> send(String topic, Event message) {
    ...
    addMetricsCallback(topic, message, MessageLogger.Direction.PUBLISH, resfultF);
  ...
  public ListenableFuture<Boolean> requeue(String topic, Event message) {
    ...
    try {
      ...
      ListenableFuture<Boolean> resfultF = apiDelegate.get().requeue(topic, requeuedEvent);
      addMetricsCallback(topic, requeuedEvent, MessageLogger.Direction.REQUEUE, resfultF);
      return resfultF;
