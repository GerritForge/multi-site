...
public class ChangeCheckerImpl implements ChangeChecker {
  ...
  public Optional<ChangeIndexEvent> newIndexEvent(
    ...
    try (Repository repo = gitRepoMgr.openRepository(notes.getProjectName())) {
      ...
      event.metaSha = getMetaSha(repo, notes);
  ...
  public boolean isUpToDate(Optional<ChangeIndexEvent> indexEventOptional) {
    ...
    ChangeNotes notes = changeNotes.get();
    long computedChangeTs = getTsFromChange(notes);

    return (computedChangeTs > indexEvent.eventCreatedOn)
        || ((computedChangeTs == indexEvent.eventCreatedOn)
            && repositoryHasRequiredShas(notes, indexEvent));
  }

  private String getBranchTargetSha(Repository repo, ChangeNotes changeNotes) throws IOException {
  ...
  private String getMetaSha(Repository repo, ChangeNotes changeNotes) throws IOException {
    String changeId = changeNotes.getProjectName() + "~" + changeNotes.getChangeId().get();
  ...
  private boolean repositoryHas(Repository repo, ChangeNotes changeNotes, String sha1ToCheck) {
    String changeId = changeNotes.getProjectName() + "~" + changeNotes.getChangeId().get();
    try {
  ...
  private boolean repositoryHasRequiredShas(ChangeNotes changeNotes, ChangeIndexEvent indexEvent) {
    ...
    try (Repository repo = gitRepoMgr.openRepository(changeNotes.getProjectName())) {
      return (indexEvent.targetSha == null
              || repositoryHas(repo, changeNotes, indexEvent.targetSha))
          && (indexEvent.metaSha == null || repositoryHas(repo, changeNotes, indexEvent.metaSha));
    } catch (IOException e) {
