...
public class ChangeCheckerImpl implements ChangeChecker {
  ...
  public boolean isUpToDate(Optional<ChangeIndexEvent> indexEventOptional) {
    ...
    if (computedChangeTs != indexEvent.eventCreatedOn) {
    ...
    try (Repository repo = gitRepoMgr.openRepository(notes.getProjectName())) {
      return (indexEvent.targetSha == null || repositoryHas(repo, notes, indexEvent.targetSha))
          && (indexEvent.metaSha == null || repositoryHas(repo, notes, indexEvent.metaSha));
    } catch (IOException e) {
