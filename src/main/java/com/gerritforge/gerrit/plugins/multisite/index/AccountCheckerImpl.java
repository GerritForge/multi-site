...
public class AccountCheckerImpl implements AccountChecker {
  ...
  public boolean isUpToDate(Optional<AccountIndexEvent> event) {
    if (event.isEmpty() || event.get().targetSha == null) {
      logger.atWarning().log(
          "Account index event is missing or has no target SHA; skipping the revision check");
      return true;
    }
  ...
  public boolean isConsistent(Account.Id accountId) {
    try (Repository repo = repoManager.openRepository(allUsers)) {
      return repo.exactRef(RefNames.refsUsers(accountId)) != null;
    } catch (IOException e) {
      logger.atSevere().withCause(e).log(
