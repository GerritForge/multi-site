...
public class AccountCheckerImpl implements AccountChecker {
  ...
  public boolean isUpToDate(Optional<AccountIndexEvent> event) {
    ...
    try (Repository repo = repoManager.openRepository(allUsers);
      ...
      Ref accountRef = getAccountRef(repo, accountEvent.accountId);
  ...
  @Override
  public boolean isConsistent(Integer accountId) {
    try (Repository repo = repoManager.openRepository(allUsers)) {
      return getAccountRef(repo, accountId) != null;
    } catch (IOException e) {
      logger.atSevere().withCause(e).log(
          "Could not check whether account %d is consistent", accountId);
      throw new RuntimeException(e);
    }
  }

  private Ref getAccountRef(Repository allUsersRepo, int accountId) throws IOException {
    try (RevWalk revWalk = new RevWalk(allUsersRepo)) {
      return allUsersRepo.exactRef(RefNames.refsUsers(Account.id(accountId)));
    }
  }
}
