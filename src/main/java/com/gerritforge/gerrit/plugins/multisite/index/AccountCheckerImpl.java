...
public class AccountCheckerImpl implements AccountChecker {
  ...
  @Inject
  AccountCheckerImpl(GitRepositoryManager repoManager, AllUsersName allUsers) {
  ...
  public boolean isUpToDate(Optional<AccountIndexEvent> event) {
    ...
    try (Repository repo = repoManager.openRepository(allUsers);
      ...
      Ref accountRef = getAccountRef(repo, Account.id(accountEvent.accountId));
  ...
  @Override
  public boolean isConsistent(Account.Id accountId) {
    try (Repository repo = repoManager.openRepository(allUsers)) {
      return getAccountRef(repo, accountId) != null;
    } catch (IOException e) {
      logger.atSevere().withCause(e).log(
          "Could not check whether account %d is consistent", accountId.id());
      throw new RuntimeException(e);
    }
  }

  private Ref getAccountRef(Repository allUsersRepo, Account.Id accountId) throws IOException {
    try (RevWalk revWalk = new RevWalk(allUsersRepo)) {
      return allUsersRepo.exactRef(RefNames.refsUsers(accountId));
    }
  }
}
