...
/**
 * Handles index flushing and acknowledgement throttling for partition-level index consumers.
 *
 * <p>The last acknowledgement timestamp is tracked independently for each index event type because
 * each type is consumed from a separate partition. The matching index is flushed and the event is
 * acknowledged only when the configured {@code index.commitIntervalMs} has elapsed for that event
 * type, or for every event when the interval is set to {@code 0}.
 */
@Singleton
public class IndexEventAckHandler {
  ...
  /** Flushes the matching index and acknowledges the event when the configured interval is due. */
  public void ackIfDue(IndexEvent event, MessageAcknowledgement<Event> acknowledgement)
      throws IOException {
    ...
    if (lastAck == null
      ...
      metrics.resetUnacknowledgedEvents(event.getType());
