...
public final class CacheKeyJsonParser {
  ...
  private Class<?> getCacheKeyClassFromDefs(CachePluginAndNameRecord cacheNameWithPlugin) {
    NavigableMap<String, Provider<CacheDef<?, ?>>> cachesByPlugin = cachesMap.byPlugin(cacheNameWithPlugin.plugin());
    if (cachesByPlugin == null) {
      throw new IllegalStateException("Unable to find any cache provided by " + cacheNameWithPlugin.plugin());
    }
    Provider<CacheDef<?, ?>> cacheDefProvider = cachesByPlugin.get(cacheNameWithPlugin.name());
    if (cacheDefProvider == null) {
      throw new IllegalStateException(
          "Unable to find definition for cache '" + cacheNameWithPlugin.name() + "' provided by " + cacheNameWithPlugin.name());
    }
