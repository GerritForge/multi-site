...
public class MultiSiteConsumerRunner implements LifecycleListener {
  ...
  // placeholder is bound and reports an acknowledgement mode that is not the real broker's. A
  // broker that is reloaded or replaced binds a new instance and drops the subscriptions made
  // against the previous one, so they have to be made again.
