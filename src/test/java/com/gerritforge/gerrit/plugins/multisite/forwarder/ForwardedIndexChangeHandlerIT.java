...
public class ForwardedIndexChangeHandlerIT extends LightweightPluginDaemonTest {
  ...
  public void deleteWithEmptyProjectNameAndNoImportedChangeLooksUpProjectFromIndex() throws Exception {
    PushOneCommit.Result changeResult = createChange();
    int changeNum = changeResult.getChange().getId().get();
    Change.Id changeId = Change.id(changeNum);

    assertThat(queryProvider.get().byLegacyChangeId(changeId)).isNotEmpty();

    objectUnderTest.index(
        "~" + changeNum,
        Operation.DELETE,
        Optional.of(new ChangeIndexEvent("", changeNum, true, "testInstanceId")));

    assertThat(queryProvider.get().byLegacyChangeId(changeId)).isEmpty();
  }

  @Test
  @GerritConfig(name = "gerrit.instanceId", value = "testInstanceId")
  @GerritConfig(name = "gerrit.importedServerId", value = IMPORTED_SERVER_ID)
  public void deleteWithEmptyProjectNameLooksUpImportedChangeFromIndex() throws Exception {
    PushOneCommit.Result changeResult = createImportedChange();
    int changeNum = changeResult.getChange().getId().get();
    Change.Id changeId = Change.id(changeNum);

    assertThat(queryProvider.get().byChangeNumber(changeId)).isNotEmpty();
    ...
    assertThat(queryProvider.get().byChangeNumber(changeId)).isEmpty();
  }
