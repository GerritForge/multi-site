...
public class MultiSiteConsumerRunnerTest {
  ...
  private static final String INDEX_GROUP_ID = "index-partition-group";
  ...
  @Test
  public void shouldSubscribeToIndexPartitionsWhenConfigured() {
    configureIndexSubscriber();
    when(eventsBrokerConfiguration.getPartitionsForTopic(INDEX_TOPIC))
        .thenReturn(
            List.of(
    ...
    when(brokerCfg.getIndexEventPartitionGroupId(INSTANCE_ID)).thenReturn(INDEX_GROUP_ID);

    runner().start();

    verify(brokerApi)
        .receiveAsyncWithPartition(INDEX_TOPIC, ChangeIndexEvent.TYPE, INDEX_GROUP_ID, consumer);
    verify(brokerApi)
        .receiveAsyncWithPartition(INDEX_TOPIC, AccountIndexEvent.TYPE, INDEX_GROUP_ID, consumer);
    verify(brokerApi)
        .receiveAsyncWithPartition(INDEX_TOPIC, ProjectIndexEvent.TYPE, INDEX_GROUP_ID, consumer);
    verify(brokerApi)
        .receiveAsyncWithPartition(INDEX_TOPIC, GroupIndexEvent.TYPE, INDEX_GROUP_ID, consumer);
    verify(brokerApi, never()).receiveAsync(eq(INDEX_TOPIC), any());
  }

  @Test
  public void shouldSubscribeToIndexTopicWhenPartitionsAreNotConfigured() {
    configureIndexSubscriber();
    when(eventsBrokerConfiguration.getPartitionsForTopic(INDEX_TOPIC)).thenReturn(List.of());
  ...
  public void shouldFailWhenAnExpectedIndexPartitionIsNotConfigured() {
    configureIndexSubscriber();
    when(eventsBrokerConfiguration.getPartitionsForTopic(INDEX_TOPIC))
        .thenReturn(List.of(ChangeIndexEvent.TYPE, ProjectIndexEvent.TYPE, GroupIndexEvent.TYPE));
    when(brokerCfg.getIndexEventPartitionGroupId(INSTANCE_ID)).thenReturn(INDEX_GROUP_ID);
  ...
  private void configureIndexSubscriber() {
