...
public class CacheKeyJsonParserTest {
  ...
  public void setUp() throws Exception {
    ...
    definePersistentCache(GERRIT, CACHE_CHANGE_KIND, ChangeKindCacheImpl.Key.class, new ChangeKindCacheImpl.Key.Serializer());
  ...
  private <K> void  definePersistentCache(String pluginName, String cacheName, Class<K> keyRawType,CacheSerializer<K> keySerializer) {
    RegistrationHandle unused =
            cacheDefMap.put(
                    pluginName,
                    cacheName,
                    Providers.of(new TestPersistentCacheDef<>(cacheName, keyRawType, Object.class, keySerializer)));
  }
  ...
  public void serializeDeserializeCacheEvictionEventWithAutoValueKeyType() {
    ChangeKindCacheImpl.Key changeKindType = ChangeKindCacheImpl.Key.create(ObjectId.zeroId(), ObjectId.zeroId(), "sample-strategy");
    String jsonEvent = gsonParser.toJson(CACHE_CHANGE_KIND, changeKindType);
    Object parsedKey = gsonParser.from(CACHE_CHANGE_KIND, jsonEvent);
  ...
  public void serializeDeserializeCacheEvictionEventForMultiplePlugins() {
    ...
    String jsonEvent1 = gsonParser.toJson(PLUGIN_NAME1 + "." + CACHE_NAME_WITH_COMPLEX_KEY_TYPE, complexKeyType1);
    String jsonEvent2 = gsonParser.toJson(PLUGIN_NAME2 + "." + CACHE_NAME_WITH_COMPLEX_KEY_TYPE, complexKeyType2);
    Object parsedKey1 =
  ...
  private static class TestPersistentCacheDef<K,V> implements PersistentCacheDef<K,V> {
    ...
    TestPersistentCacheDef(String name, Class<K> keyClass, Class<V> valueClass, CacheSerializer<K> keySerializer) {
      this.name = name;
