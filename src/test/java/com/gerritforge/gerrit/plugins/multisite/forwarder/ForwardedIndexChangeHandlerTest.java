...
public class ForwardedIndexChangeHandlerTest {
  ...
  public static final int MAX_INDEX_TRIES = 1;
  ...
  public void changeIsIndexedWhenUpToDate() throws Exception {
    ...
    verify(indexerMock, times(MAX_INDEX_TRIES)).index(any(ChangeNotes.class));
  }
  ...
  public void changeIsStillIndexedEvenWhenOutdated() throws Exception {
    ...
    handler.index(
        TEST_CHANGE_ID,
        Operation.INDEX,
        Optional.of(new ChangeIndexEvent("foo", MAX_INDEX_TRIES, false, "instance-id")));
    verify(indexerMock, times(MAX_INDEX_TRIES)).index(any(ChangeNotes.class));
  }
  ...
  public void changeIsIndexeAtFirstRetryWhenInitiallyInconsistent() throws Exception {
    ...
    handler.index(
        ...
        Optional.of(new ChangeIndexEvent("foo", MAX_INDEX_TRIES, false, "instance-id")));
    verify(indexerMock, never()).index(any(ChangeNotes.class));
    verify(indexExecutorMock, times(MAX_INDEX_TRIES))
  ...
  private void setupChangeAccessRelatedMocks(
    ...
    OngoingStubbing<Boolean> changeConsistentCall = when(changeCheckerPresentMock.isConsistent());
