...
function create_kinesis_streams {
  ...
  do
    if [ "$stream" = "gerrit_index" ] && [ "$index_partitions" -gt 0 ]; then
      create_kinesis_stream "$stream" "$index_partitions"
    else
      create_kinesis_stream "$stream" 1
    fi
  done
}
...
function download_gh_artifact_from_ci {
...
case "$1" in
 ...
 "--broker-type" )
  ...
  "--broker-manual-ack" )
    ...
    shift
    shift
    if [ "$BROKER_MANUAL_ACK" != "true" ] && [ "$BROKER_MANUAL_ACK" != "false" ]; then
      echo >&2 "broker manual-ack: '$BROKER_MANUAL_ACK' not valid. Please supply 'true' or 'false'. Aborting"
      exit 1
    fi
  ;;
  "--sudo" )
...
if [ $DOWNLOAD_WEBSESSION_PLUGIN = "true" ];then
  ...
  download_gh_artifact_from_ci websession-broker
...
echo "Downloading pull-replication plugin $GERRIT_BRANCH"
  download_gh_artifact_from_ci pull-replication
