...
function copy_replication_remote_file {
  ...
  echo "Replacing variables for file $file and copying to $CONFIG_TEST_SITE/etc/replication/$file_name"

  mkdir -p $CONFIG_TEST_SITE/etc/replication
  cat $file | envsubst | sed 's/#{name}#/${name}/g' > $CONFIG_TEST_SITE/etc/replication/$file_name
}
...
function deploy_config_files {
  ...
  copy_replication_remote_file $CONFIG_TEST_SITE_1 $GERRIT_SITE1_HTTPD_PORT $LOCATION_TEST_SITE_1 $GERRIT_SITE1_SSHD_PORT $GERRIT_SITE2_HTTPD_PORT $GERRIT_SITE1_HOSTNAME $GERRIT_SITE2_HOSTNAME $GERRIT_SITE1_REMOTE_DEBUG_PORT $GERRIT_SITE1_INSTANCE_ID $GERRIT_SITE2_INSTANCE_ID
  ...
  copy_replication_remote_file $CONFIG_TEST_SITE_2 $GERRIT_SITE2_HTTPD_PORT $LOCATION_TEST_SITE_2 $GERRIT_SITE2_SSHD_PORT $GERRIT_SITE1_HTTPD_PORT $GERRIT_SITE1_HOSTNAME $GERRIT_SITE2_HOSTNAME $GERRIT_SITE2_REMOTE_DEBUG_PORT $GERRIT_SITE2_INSTANCE_ID $GERRIT_SITE1_INSTANCE_ID
}
