...
ORIGINAL_DOCKER_CONFIG=${HOME:-$(eval echo "~$(id -un)")}/.docker
[ -d "${ORIGINAL_DOCKER_CONFIG}" ] && cp -R "${ORIGINAL_DOCKER_CONFIG}/." "${DOCKER_CONFIG}/"
...
compose -f ${DEPLOYMENT_LOCATION}/docker-compose.yaml up --no-recreate -d gerrit1 gerrit2

echo "Waiting for services to start (and being healthy) and calling e2e tests"
compose -f ${DEPLOYMENT_LOCATION}/docker-compose.yaml up --no-recreate --no-start tester
compose -f ${DEPLOYMENT_LOCATION}/docker-compose.yaml ps -a
...
compose -f ${DEPLOYMENT_LOCATION}/docker-compose.yaml up --no-recreate tester
